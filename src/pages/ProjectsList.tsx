...
const ProjectsList = (props: RouteParams): JSX.Element => {
    ...
    const [showSkeleton, setShowSkeleton] = useState<boolean | undefined>(
        undefined
    );
    ...
    useEffect(() => {
        if (token !== undefined && token !== null) {
            const q = query(
                ...
                where("admin", "==", match.params.adminId)
            );
    ...
    const showUpdateAlert = (updated: Date) => {
        const offset = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
        ...
        <IonPage className={`custom-project-list`}>
            ...
            <IonContent className={`custom-content`}>
                ...
                <AddProject
                    ...
                    {...{ setToastType, setShowToast, setToastMessage }}
                    {...props}
                />
                <MyToast
                    showToast={showToast}
