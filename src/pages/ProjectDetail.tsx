...
const ProjectDetail = (props: RouteParams): JSX.Element => {
    ...
    const [showAccordion, setShowAccordion] = useState<ACCORDION>(
        ACCORDION.CLOSE
    );
    const [showToast, setShowToast] = useState<boolean>(false);
    const [toastMessage, setToastMessage] = useState<string>("");
    const [toastType, setToastType] = useState<ToastType>(ToastType.error);
    const [showCircle, setShowCircle] = useState<boolean | undefined>(
        undefined
    );
    const [stepHeight, setStepHeight] = useState<number>(0);
    const [transitionEnd, setTransitionEnd] = useState<boolean | undefined>(
        undefined
    );
    ...
    const [logoNoText, setLogoNoText] = useState<string>();

    useEffect(() => {
        (async () => {
            const url = await getPngUrl("arccon-logo-no-text.png");
            setLogoNoText(url);
        })();
    }, []);
    ...
    useEffect(() => {
        if (loaded && transitionEnd !== undefined) {
            content.current?.scrollToPoint(
                0,
                boxTop - content.current?.scrollHeight + stepHeight,
                1000
            );
        }
    ...
    useEffect(() => {
        if (token !== undefined && token !== null) {
            const unsubscribe = onSnapshot(
                doc(db, "Projects", match.params.projectId),
                (result) => {
                    const newProject = {
                        id: result.id,
    ...
    return (
        <IonPage className={`custom-project-detail`}>
            <IonHeader>
                <IonToolbar className={`custom-toolbar`}>
                    <IonButtons slot={`start`}>
                        <IonBackButton
                            defaultHref={`/${match.params.adminId}/project/list`}
                        />
                    </IonButtons>
            ...
            >
                <IonAccordionGroup
                    className={`custom-accordion-group`}
                    value={showAccordion.toString()}
                    style={{ position: `sticky`, top: `12px`, zIndex: 1 }}
                >
                    <IonAccordion
                        onTransitionEnd={() =>
                            setTransitionEnd((prev) => !prev)
                        }
                        className={`custom-accordion`}
                        value={ACCORDION.OPEN.toString()}
                    >
                        <IonItem
                            slot={`header`}
                            color={`medium`}
                        ...
                        >
                            ...
                            {project !== undefined && (
                                <>
                                    <IonItem
                                        className={`description-item custom-item-paragraph`}
                                        color={`transparent`}
                                    >
                                    ...
                                    <IonItem
                                        className={`description-item custom-item ${
                                            project.environmental_survey
                                                ? "custom-success-icon"
                                                : "custom-danger-icon"
                                        }`}
                                        color={`transparent`}
                ...
                {project !== undefined && (
                    <VerticalLinearStepper
                        ...
                        environmentalSurvey={project.environmental_survey}
                        ...
                        renderControl={
                            isBoss ||
                            user.data?.id === match.params.adminId ||
                            true
                        }
                    />
                )}
