...
interface itemProps {
    ...
    handleIsOpen: (
        id: string
    ) => (e: React.MouseEvent<HTMLIonButtonElement, MouseEvent>) => void;
}
...
const Item = (props: itemProps): JSX.Element => {
    ...
    return (
        <IonButton
            className={`${classes.customButton} ion-margin-bottom`}
            disabled={handleDisable(department.name)}
            onClick={handleIsOpen(department.id)}
            expand={`block`}
            {...btnProps}
        >
...
const Setup = (props: RouteComponentProps): JSX.Element => {
    ...
    useEffect(() => {
        (async () => {
            const url = await getPngUrl("arccon-logo.png");
            setLogo(url);
        })();
    }, []);
