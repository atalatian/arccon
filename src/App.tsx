                    ...
                    await PushNotifications.addListener(
                        ...
                        (action) => {
                            ...
                            const projectId = notification.data
                                .projectId as string;
                            history.push(
                                `/${adminId}/project/${projectId}/detail`
                            );
                        }
                    );
            ...
            if (token !== undefined && token !== null) {
                if (Capacitor.isPluginAvailable("PushNotifications")) {
                    ...
                    if (permStatus.receive === "denied") {
                        setToastType(ToastType.error);
                        setToastMessage(
                            "Permission is not granted for push notifications"
                        );
                        setShowToast(true);
                    }
                    ...
                    Notification.requestPermission().then((permission) => {
                        if (permission === "granted") {
                            ...
                            getToken(messaging, {
                                vapidKey:
                                    "BAFv2yeCCeuhFakQIyNuF6GfVtV7Cq-r1P1dd_Kw0cEJrCOK6YHUJk1mj7txi86ANCWrTJEa-VWcuTO-LNZmUM4",
                            })
        ...
        <IonApp>
            <IonReactRouter>
                <IonRouterOutlet animated={true}>
                    ...
                    <Route
                        exact
                        path={`/:adminId/project/list`}
                        component={ProjectsList}
                    />
                    <Route exact path={`/project/add`} component={AddProject} />
                    <Route
                        exact
                        path={`/:adminId/project/:projectId/detail`}
